...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            size = len(self.stack)
            self.dependencies.append((self.stack[size-1], self.stack[size-2]))
            self.stack.pop(size-2)
        elif transition == "RA":
            size = len(self.stack)
            self.dependencies.append((self.stack[size-2], self.stack[size-1]))
            self.stack.pop(size-1)
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    size = 0
    while size < len(unfinished_parses):
        batch = unfinished_parses[size:size+batch_size]
        while len(batch) > 0:
            transitions = model.predict(batch)
            for partial_parse, transition in zip(batch, transitions):
                partial_parse.parse_step(transition)
            batch = [batch[i] for i in range(len(batch)) if len(batch[i].stack) > 1 or len(batch[i].buffer) > 0]
        size += batch_size
