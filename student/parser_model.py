...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight_shape = [self.n_features*self.embed_size, self.hidden_size]
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(torch.empty(self.embed_to_hidden_weight_shape)))

        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(torch.empty(1, self.hidden_size)))
        self.dropout = nn.Dropout(self.dropout_prob)

        self.hidden_to_logits_weight_shape = [self.hidden_size, self.n_classes]
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(torch.empty(self.hidden_to_logits_weight_shape)))
    ...
    def embedding_lookup(self, w):
        ...
        for i in range(w.shape[0]):
            embeddings = self.embeddings[w[i]]
            x[i] = embeddings.reshape((1, -1))
    ...
    def forward(self, w):
        ...
        layer_1_input = self.embedding_lookup(w)
        layer_1_output = torch.mm(layer_1_input, self.embed_to_hidden_weight) + self.embed_to_hidden_bias
        layer_1_output = self.dropout(layer_1_output)

        logits = torch.mm(layer_1_output, self.hidden_to_logits_weight) + self.hidden_to_logits_bias
